...
(function () {
  ...
  // Extracted row mappers (Phase 1: src/sync/rows.js, loaded by index.html
  // BEFORE this file) pulled off the namespace in ONE block at the top of
  // this IIFE — the same load-bearing position as app.js's pull block, and
  // for the same reason: a const does not hoist, so the pull must run before
  // anything that could call these names. It sits above the config check
  // deliberately — a missing script tag fails loudly here, in every mode,
  // rather than silently in whichever mode happens to call a mapper first.
  const { athleteToRow, rowToAthlete, progressToRow, rowToProgress } =
    globalThis.STSD.sync;
  // The push queue's ONE instance (src/sync/push-queue.js, loaded before
  // this file) — same load-bearing position, same loud failure.
  const { pushQueue } = globalThis.STSD.sync;
  if (!pushQueue) {
  ...
  // The row mappers (athleteToRow, rowToAthlete, progressToRow,
  // rowToProgress) moved to src/sync/rows.js (Phase 1 extraction); their
  // namespace pull lives at the TOP of this IIFE — see the comment there
  // for why position is load-bearing. window.Cloud still exports
  // rowToAthlete/rowToProgress below for app.js's realtime handlers.
  ...
  // The push queue (debounce/flush and the retry machinery) moved to
  // src/sync/push-queue.js — the deferred brick (design doc §5). The ONE
  // instance is constructed there at load and pulled at the top of this
  // IIFE; the destructured names below are what the Cloud surface exports.
  window.Cloud = {
    enabled: true,
    sb,
    // Auth
